...
begin
  try
    ...
    begin
      ...
      LOG(5,'敏盛:資料庫連線完成,耗時:'+inttostr(TTIME)+' ms',LightGreen);
      //==========================================================================================會跑到以下,代表資料庫連線都正常
      Target_LogTime:= windows.GetTickCount;

      LOG(5,'敏盛:[DTMF]='+CTI_Data.DTMF_Code+'[DATA]='+CTI_Data.MSG_DATA,LightGreen);
      LOG(5,'敏盛:[CallID]='+trim(CTI_Data.CallerID)+
      ...
      LOG(5,'敏盛:資料於 '+TDBObject.Target_Query.FieldByName('cti_time').AsString+' 時進入CTI並檢察完成',LightGreen);
      LOG(5,'敏盛:[MSG]='+trim(CTI_Data.MSG_MSGText)+
      ...
      begin
        ...
        LOG(5,'敏盛:[收縮壓]='+inttostr(TDBObject.Target_Query.FieldByName('vital_sign_1').AsInteger)+
    ...
    LOG(5,'敏盛:'+CTI_Data.DLL_ProcessMSG,LightGreen);
    exit;
  end;
  result:=true;
  ...
  LOG(5,'敏盛:界接資料轉換完成,耗時:'+inttostr(TDBObject.Target_LogTime)+' ms',LightGreen);
  CTI_Data.Process:=1;
...
begin
  ...
  NowIsLink:=true;
  if ShowLinkOk then LOG(5,'敏盛:SQL Server連線完成,耗時:'+inttostr(Target_LogTime)+' ms',LightGreen);
end;
...
begin
  ...
  Target_LogTime:=windows.GetTickCount;
//===========================================================
  if ShowLinkOk then LOG(5,'敏盛:敏盛醫院介接動態連結函式庫初始化',LightGreen);
...
//===========================================================CTI DB
  ...
  if ShowLinkOk then LOG(5,'敏盛:敏盛醫院介接動態連結函式庫載入完成,耗時:'+inttostr(Target_LogTime)+' ms',LightGreen);
end;
...
//===========================================================CTI DB 開啟
  try
    if ShowLinkOk then LOG(5,'敏盛:載入MS SQL驅動程式',LightGreen);
    if ShowLinkOk then LOG(5,'敏盛:IP='+C_SVR+' ID='+C_USN+' PWD='+C_PWD+' CAT='+C_CAT,LightGreen);
    if ShowLinkOk then LOG(5,'敏盛:DB連線中',LightGreen);
    Target_LogTime:=windows.GetTickCount;
    ...
    LOG(5,'敏盛:DB連線失敗',LightGreen);
    Err_flag:=1;
  end;
//===========================================================進入工作模式
  if Err_flag=0 then
  begin
     if ShowLinkOk then LOG(5,'敏盛:資料庫連線完成,進入工作模式',LightGreen);
  end;
end;
...
begin
  ...
  NowIsLink:=true;
  if ShowLinkOk then LOG(5,'敏盛:SQL Server連線完成,耗時:'+inttostr(CTI_LogTime)+' ms',LightGreen);
end;
...
//===========================================================CTI DB
  ...
  if ShowLinkOk then LOG(5,'敏盛:敏盛醫院介接動態連結函式庫載入完成,耗時:'+inttostr(LogTime)+' ms',LightGreen);
