...
type
  ...
  function  GetMeasureString(Vital1,Vital2,Vital3,Vital4:integer):string; //輸入參數，解碼出血壓/血糖/體溫/PEAK/體重
implementation
...
begin
   ...
   try
      ...
      TDBObject.Target_Query.SQL.Text:='select top 1 * from MCH_Vital';
...
function  GetMeasureString(Vital1,Vital2,Vital3,Vital4:integer):string;
var
  TempFloat:double;
  TempFloat2:double;
begin
   //===================================體溫        000,000,TT.T
     if (Vital1=0)and(Vital2=0)and(Vital3<>0) then
     begin
        TempFloat:= Vital3 / 10;
        result:=format('[體溫]=%2.1f 度C',[TempFloat]);
        exit;
     end;
   //===================================體重        0或1,體重Mod100,BMI
     if ((Vital1=0)or(Vital1=1))and(Vital3<>0) then
     begin
        TempFloat:= (Vital1*100)+ (Vital2/10); //體重
        TempFloat2:= Vital3/10;
        if Vital3=999 then  //代表無BMI
          result:=format('[體重]=%3.1f KG',[TempFloat])
        else
          result:=format('[體重]=%3.1f KG [BMI]=%3.1f',[TempFloat,TempFloat2]);
        exit;
     end;
   //===================================Peak Flow   PPP,V.VV,000
     if (Vital1<>0)and(Vital2<>0)and(Vital3=0) then
     begin
        TempFloat:= Vital2 / 100;
        result:=format('[PEF]=%d [FEV1]=%1.2f l',[Vital1,TempFloat]);
        exit;
     end;
   //===================================血壓        SSS,DDD,HHH
     if (Vital1>1)and(Vital2<>0)and(Vital3<>0) then
     begin
        result:=format('[收縮壓]=%d [舒張壓]=%d [心跳]=%d',[Vital1,Vital2,Vital3]);
        exit;
     end;
   //===================================血糖        GGG,000,000
     if (Vital4<>0) then
     begin
        result:=format('[血糖]=%d',[Vital4]);
        exit;
     end;
end;
...
begin
  try
    ...
    begin
      ...
      Target_Query.SQL.Text:='select top 1 * from MCH_Vital';
      Target_Query.Open;
      for i:=0 to 100 do
      begin
        sleep(50);
      ...
      LOG(5,'門諾:資料庫連線完成,耗時:'+inttostr(TTIME)+' ms',LightGreen);
      //==========================================================================================會跑到以下,代表資料庫連線都正常
      Target_LogTime:= windows.GetTickCount;

      LOG(5,'門諾:[DTMF]='+CTI_Data.DTMF_Code+'[DATA]='+CTI_Data.MSG_DATA,LightGreen);
      LOG(5,'門諾:[CallID]='+trim(CTI_Data.CallerID)+
      ...
      LOG(5,'門諾:資料於 '+TDBObject.Target_Query.FieldByName('cti_time').AsString+' 時進入CTI並檢察完成',LightGreen);
      LOG(5,'門諾:[MSG]='+trim(CTI_Data.MSG_MSGText)+
      ...
      begin
        ...
        LOG(5,'門諾:'+GetMeasureString(Target_Query.FieldByName('vital_sign_1').AsInteger,
                                       Target_Query.FieldByName('vital_sign_2').AsInteger,
                                       Target_Query.FieldByName('vital_sign_3').AsInteger,
                                       Target_Query.FieldByName('vital_sign_4').AsInteger,
                                       ),LightGreen);
{
        LOG(5,'門諾:[收縮壓]='+inttostr(TDBObject.Target_Query.FieldByName('vital_sign_1').AsInteger)+
    ...
    LOG(5,'門諾:'+CTI_Data.DLL_ProcessMSG,LightGreen);
    exit;
  end;
  result:=true;
  ...
  LOG(5,'門諾:界接資料轉換完成,耗時:'+inttostr(TDBObject.Target_LogTime)+' ms',LightGreen);
  CTI_Data.Process:=1;
...
begin
  ...
  NowIsLink:=true;
  if ShowLinkOk then LOG(5,'門諾:SQL Server連線完成,耗時:'+inttostr(Target_LogTime)+' ms',LightGreen);
end;
...
begin
  ...
  Target_LogTime:=windows.GetTickCount;
//===========================================================
  if ShowLinkOk then LOG(5,'門諾:門諾醫院介接動態連結函式庫初始化',LightGreen);
...
//===========================================================CTI DB
  ...
  if ShowLinkOk then LOG(5,'門諾:門諾醫院介接動態連結函式庫載入完成,耗時:'+inttostr(Target_LogTime)+' ms',LightGreen);
end;
...
//===========================================================CTI DB 開啟
  try
    if ShowLinkOk then LOG(5,'門諾:載入MS SQL驅動程式',LightGreen);
    if ShowLinkOk then LOG(5,'門諾:IP='+C_SVR+' ID='+C_USN+' PWD='+C_PWD+' CAT='+C_CAT,LightGreen);
    if ShowLinkOk then LOG(5,'門諾:DB連線中',LightGreen);
    Target_LogTime:=windows.GetTickCount;
    ...
    LOG(5,'門諾:DB連線失敗',LightGreen);
    Err_flag:=1;
  end;
//===========================================================進入工作模式
  if Err_flag=0 then
  begin
     if ShowLinkOk then LOG(5,'門諾:資料庫連線完成,進入工作模式',LightGreen);
  end;
end;
...
begin
  ...
  NowIsLink:=true;
  if ShowLinkOk then LOG(5,'門諾:SQL Server連線完成,耗時:'+inttostr(CTI_LogTime)+' ms',LightGreen);
end;
...
//===========================================================CTI DB
  ...
  if ShowLinkOk then LOG(5,'門諾:門諾醫院介接動態連結函式庫載入完成,耗時:'+inttostr(LogTime)+' ms',LightGreen);
