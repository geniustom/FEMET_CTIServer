...
function OutputADOTOCSV(Query:TOraQuery):TStringList;
var
  myStringlist:Tstringlist;
...
begin
  ...
  DataList.Delimiter:=#13;

  myStringlist.Add('編號');
  for nCnt:=0 to Query.FieldCount -1 do
     begin
         myStringlist.Add(Query.Fields[nCnt].FieldName);
     end;

  DataList.Add(myStringlist.DelimitedText);
  ...
  begin
    CNT:=CNT+1;
    myStringlist.Clear;
    myStringlist.Add(inttostr(CNT));
    for nCnt:=0 to Query.FieldCount -1 do
    begin
      myStringlist.Add(Query.Fields[nCnt].AsString);
    end;
    DataList.Add(myStringlist.DelimitedText);
    Query.Next;
  end;
...
function OutputTOCSV(Query:TOraQuery):TStringList;
var
  myStringlist:Tstringlist;
  DataList:Tstringlist;
  nCnt:integer;
...
begin
   ...
   begin
    ...
    Mail(ParmStr.Strings[8],ParmStr.Strings[7],OutputTOCSV(OraQuery1));
   end;
end;
